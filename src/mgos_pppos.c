...
static void mgos_pppos_dispatch_once(struct mgos_pppos_data *pd) {
  ...
  switch (pd->state) {
    ...
    case PPPOS_SETUP: {
      ...
      add_cmd(pd, mgos_pppos_at_cb, 500, "AT");
      // add_cmd(pd, NULL, 0, "ATH");//ROZLACZA POLACZENIA
      add_cmd(pd, NULL, 500, "ATE0");//ECHO disable
      ...
      add_cmd(pd, mgos_pppos_ati_cb, 500, "ATI");//Request modem information
      add_cmd(pd, mgos_pppos_gsn_cb, 500, "AT+GSN");//Request IMEI
      add_cmd(pd, mgos_pppos_cimi_cb, 500, "AT+CIMI");//Request IMSI
      add_cmd(pd, mgos_pppos_ccid_cb, 500, "AT+CCID");//Request ICCID ??
      add_cmd(pd, mgos_pppos_cpin_cb, 500, "AT+CPIN?");//Request PIN need ?

      if (mgos_sys_config_get_pppos_cfun_cycle()) {
        add_cmd(pd, NULL, 500, "AT+CFUN=0"); /* Offline */
      }

//ADD INIT BG95
      add_cmd(pd, NULL, 500, "AT+QCFG=\"nwscanmode\",0,1");//Configure automatic searching of all Radio Access Technology (RAT) types, to include GSM (2G) and LTE (Cat M1)
      add_cmd(pd, NULL, 500, "AT+QCFG=\"nwscanseq\",020103,1");//Configure the RAT search sequence in the following order: LTE Cat M1 -> GSM -> LTE Cat NB1
      add_cmd(pd, NULL, 500, "AT+QCFG=\"band\",0,100002000000000F0E189F,10004200000000090E189F");//Configure the device to search all available frequency bands
      add_cmd(pd, NULL, 500, "AT+QCFG=\"iotopmode\",0,1"); //Configure the modem to only use LTE Cat-M1, not NB-IoT
//END INIT BG95-M3

      add_cmd(pd, NULL, 500, "AT+%s=0", reg_cmd); // AT+CREG=0 Disable network registration unsolicited result code. You will need to manually check the network registration status.
      bool ok = false;
      ...
      add_cmd(pd, NULL, 500, "AT+CGDCONT=1,\"IP\",\"%s\"", pd->cfg->apn);
      add_cmd(pd, NULL, 500, "AT+CFUN=1"); /* Online */

      add_cmd(pd, mgos_pppos_creg_cb, 500, "AT+%s?", reg_cmd); //AT+CREG? Check the network registration state AT+CEREG? for LTE !!!
      add_cmd(pd, NULL, 500, "AT+COPS=3,2"); /* Numeric operator format. */
      add_cmd(pd, mgos_pppos_cops_cb, 500, "AT+COPS?"); //moze byc problem bo na koncu komendy doszla band technology
      add_cmd(pd, NULL, 500, "AT+COPS=3,0"); /* Long alphanumeric format. */
      add_cmd(pd, mgos_pppos_cops_cb, 500, "AT+COPS?");
      add_cmd(pd, mgos_pppos_csq_cb, 500, "AT+CSQ");

      
      add_cmd(pd, NULL, 500, "AT+CGACT=1,1");//Activate the PDP (packet data protocol) context
      add_cmd(pd, NULL, 500, "AT+CGPADDR");//Check IP
      ...
      if (cmd_done) {
        ...
        if (cmd_res) {
          ...
          if (pd->cmd_idx >= pd->num_cmds ||
            ...
            if (pd->delay == 0) pd->delay = mgos_uptime() + 0.2;
            mgos_pppos_set_state(pd, PPPOS_CMD);
          }
        } else {
          free_cmds(pd, false /* ok */);
